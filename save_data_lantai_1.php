...
if (!empty($barcodeResult) && !empty($selectedActions)) {
    ...
    $queryArea = "SELECT settingarea_id, AREA FROM db_mobile_collection.setting_area WHERE AREA = 'LANTAI SATU'";
    $resultArea = $connectionServernew->query($queryArea);
    $areaData = $resultArea->fetch_assoc();

    $queryDetailsArea = "SELECT settingdetailsarea_id, area_id, details_area FROM db_mobile_collection.settingdetails_area WHERE area_id IN (SELECT settingarea_id FROM db_mobile_collection.setting_area WHERE AREA = 'LANTAI SATU')";
    ...
    $query = "INSERT INTO penilaian_kinerja_office_boy_lantai_1 (
        kinerja_id, staff_id, areatempat_id, areadetail_id, pekerjaan_id, waktu, jam, foto_before, foto_after, STATUS
    ) VALUES (
